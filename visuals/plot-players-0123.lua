...
function M.update()
	...
	for i=0, winWidth do
		...
		-- get the value, offset by +1
		-- flip the value's sign, and multiply by a quarter the window height
		-- we use a quarter because the window is 2 plots tall, and each plot has 2 halves
		M.a0[i] = (-1 * array0:getAt(arrayIdx) * ampScale) + (0.25*winHeight)
		M.a1[i] = (-1 * array1:getAt(arrayIdx) * ampScale) + (0.25*winHeight)
		
		-- on the second row, plots 2 and 3 must be offset by half the window height
