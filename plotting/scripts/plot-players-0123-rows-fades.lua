...
function M.update()
	...
	for i=0, M.num do
		...
		local ampScale = 0.1 * winHeight -- 0.125 is full range
		-- get the value in the table, multiply by ampScale
		-- 
		-- 
		M.a0[i] = (-1 * array0:getAt(arrayIdx) * ampScale) + (0.125*winHeight)
		M.a1[i] = (-1 * array1:getAt(arrayIdx) * ampScale) + (0.375*winHeight)
		
		-- 
		M.a2[i] = (-1 * array2:getAt(arrayIdx) * ampScale) + (0.625*winHeight)
		M.a3[i] = (-1 * array3:getAt(arrayIdx) * ampScale) + (0.875*winHeight)
	end
...
function M.draw()
	...
	for i=0, M.num-1 do
		...
		ofSetColor(brightness, brightness, brightness)
		ofDrawLine(i, M.a3[i], i+1, M.a3[i+1])
